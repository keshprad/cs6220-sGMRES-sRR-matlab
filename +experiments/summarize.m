...
quartiles = ratioQuartiles(classicalCore,sketchedCore,comparable);
result.ComparableCoreSpeedupQ25 = quartiles(1);
result.ComparableCoreSpeedupQ75 = quartiles(2);
...
function values = ratioQuartiles(classical,sketched,selection)
ratios = classical(selection)./sketched(selection);
ratios = sort(ratios(isfinite(ratios)));
if isempty(ratios)
    values = [NaN NaN];
elseif numel(ratios) == 1
    values = [ratios ratios];
else
    values = interp1(1:numel(ratios),ratios, ...
        1+(numel(ratios)-1)*[0.25 0.75]);
end
end
...
row = struct( ...
    ...
    ComparableCoreSpeedupQ25=NaN, ...
    ComparableCoreSpeedupQ75=NaN, ...
